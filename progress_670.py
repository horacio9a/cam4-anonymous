...
class ProgressFormatter:
    ...
    FORMATS: _TFormat = tuple(list(_stringformatter.parse(fmt)) for fmt in (
        "[download] => {path} {written} ({elapsed} @ {speed})",
    ...
    def format_filesize(cls, size: float, suffix: str = "") -> str:
        ...
        if size < 2**20:
            return f"{cls._round(size / 2**10, 2):.2f} KB{suffix}"
        if size < 2**30:
            return f"{cls._round(size / 2**20, 2):.2f} MB{suffix}"
        if size < 2**40:
            return f"{cls._round(size / 2**30, 2):.2f} GB{suffix}"

        return f"{cls._round(size / 2**40, 2):.2f} TB{suffix}"
